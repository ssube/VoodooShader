...
public:
   ...
   */
  File(_In_ Core *core, _In_ String name);
  ...
  virtual IImageRef OpenImage(void);
   ...
   * -1 for all. @param buffer The buffer to be read into (may be null, see notes).
   * @return The number of bytes read. @note If @arg buffer is null, the number of
   ...
   * @return Success of the write operation. @note If @p buffer is null, @p count
...
public:
   ...
   * placed in. Must already be allocated, or null. @return The number of bytes
   * retrieved (or, if @p buffer is null, the number that would be retrieved).
