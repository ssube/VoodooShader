...
namespace VoodooShader
{
 ...
 /**
  * Classes implemented entirely within the core module (these do not derive from
  * IObject). These core classes are available to all Voodoo modules and addons,
  * but use requires linking against the core library. These provide most of the
  * basic functions needed by the framework. @addtogroup VoodooCore Voodoo/Core @{
  */
 class Converter;
 class Core;
 class Exception;
 ...
 /**
  * Interface classes, providing standard access to various dynamic objects. These
  * are not implemented within the core but are instead dynamically linked at
  * runtime. All are derived from IObject.
  */
 class IObject;
 class IAdapter;
 class IFileSystem;
 ...
 /**
  -------------------------------------------------------------------------------------------------------------------
  * Generic structs for passing simple data sets. Structs have no ctor or dtor, nor methods;
  * they should be created with the <code>{m,n,o}</code> syntax. Macros may be available to construct some struct types.
  *
  -------------------------------------------------------------------------------------------------------------------
  */
 struct TextureDesc;
 ...
 /**
  * Collection types for most common objects. These provide specialized containers
  * and save typing. ;
  * Shader collection
  */
 ...
 typedef std::pair<ModulePtr, int> ClassID;  /* !< Defines the necessary data to create a class (source module
              * and class number). */
 ...
 /**
  * Texture formats for use by @ref VoodooShader::Texture "Textures", describing
  * the layout and size of the texture data. These may not be implemented by the
  * underlying graphics API exactly as they are indicated here, but the available
  * components and sizes are guaranteed to be equal to or greater than the
  * indicated values. Further information on texture formats and depth may be found
  * on the @ref Textures "texture formats page".
  */
 enum TextureFormat
 {
 ...
 enum ParameterType
 {
  ...
  // Samplers
  PT_Sampler1D= 0x101,  /* !< One-dimensional sampler (for a 1D texture, see @ref texturetypes "texture
          * types" for more info) */
  PT_Sampler2D= 0x102,  /* !< Two-dimensional sampler (for a 2D texture, see @ref texturetypes "texture
          * types" for more info) */
  PT_Sampler3D= 0x103,  /* !< Three-dimensional sampler (for a 3D/volume texture, see @ref texturetypes
          * "texture types" for more info) */
 ...
 enum ProgramStage
 {
  ...
  PS_Vertex = 0x01,   /* !< Vertex program stage (usually supported, see @ref programstages "program
          * stages" for more info) */
  PS_Fragment = 0x02,   /* !< Fragment program stage (usually supported, see @ref programstages "program
          * stages" for more info) */
  PS_Geometry = 0x03,   /* !< Geometry program stage (sometimes supported, see @ref programstages "program
          * stages" for more info) */
  PS_Domain = 0x04,   /* !< Domain program stage (not always supported, see @ref programstages "program
          * stages" for more info) */
  PS_Hull  = 0x05,   /* !< Hull program stage (not always supported, see @ref programstages "program
          * stages" for more info) */
 ...
 /**
  * Log message levels. These are set up to quickly filter based on various flags
  * using binary logic. Flags are ORed together, with lower-priority flags having
  * all higher-priority flags set. When checking a log call, the condition is:
  * @code LogLevel maskedLevel = level & storedLevel;
  * if ( (maskedLevel & LL_Origin) && (maskedLevel & LL_Severity) ) { // Log
  * @endcode
  */
 enum LogLevel
 {
 ...
 enum LogFlags { LF_Unknown = 0x00, LF_Flush = 0x01, LF_Max = 0x7FFFFFFF };
 ...
 enum VariableType { VT_Normal = 0x00, VT_State = 0x01, VT_System = 0x10, VT_Max = 0x7FFFFFFF };
