...
{
     /**
      * @page voodoo_graphics_shaders Shaders
      *
      ...
      *
      * @section voodoo_graphics_shaders_stages Program Stages
      *
      * Each program is designed to control a single specific stage of the render pipeline, typically handling how a data
      * type is transformed (geometry, vertex, fragment/pixel). The two most common stages, vertex and fragment, work with
      * existing data and transform it. Both have been supported since OpenGL 1.4 and DirectX 8 in some form, and almost all
      * systems today support both. Given Voodoo's focus, it is safe to assume both stages are supported (the exact level
      * of support may vary, however).
      *
      * The fragment stage handles each pixel (or fragment) as it is rendered. The fragment program handles lighting in
      ...
      *
      * The vertex stage handles each vertex, transforming them into view space and sometimes calculating lighting or
      * surface normals.
      *
      ...
      *
      * @section voodoo_graphics_shaders_system Shader System
      *
      * In addition to the standard Cg compiler, Voodoo adds a few additional features to the shader system:
      *
      * @subsection voodoo_graphics_shaders_system_parameters Shader Parameters
      *
      ...
      * useful for things like system time, common matrices, random values or other data that changes each frame but not
      * within the frame and does not vary between shaders.
      *
      * @subsection voodoo_graphics_shaders_targets Shader Targets
      *
      ...
      * texture. The results of the technique or pass, usually sent to the @p lastshader or @p lastpass texture, will
      ...
      * technique dx9_high
      * {
      *     pass diffuse < string target=":buffer_diffuse"; >
      *     {
      *         ...
      *     }
      ...
      * other effects. If no target is specified in the technique or pass annotations, the shader linker uses the texture
      * currently bound to the core's stage textures. These may be set using ICore::SetStageTexture().
      *
      * @page voodoo_graphics_params Parameters
      *
      ...
      *
      * The shaders used in Voodoo are capable of accessing a number of texture types, and Voodoo is able to load a wide
      * variety of texture formats. This provides a very powerful system and allows you to use a great variety of textures.
      *
      * @section voodoo_textures_types Texture Types
      *
      * Three basic texture types are supported in Voodoo:
      *     @li 1-dimensional texture rows (a simple row of pixels, 1 by x)
      *     @li 2-dimensional texture planes (the most common type, a rectangle of pixels, x by y)
      *     @li 3-dimensional texture volumes (effectively a number of 2D textures stacked in slices, x by y by z)
      *
      ...
      *
      * <table>
      *     <tr><th>Voodoo Format</th> <th>DirectX Format</th> <th>OpenGL Format</th> <th>RT/Copy/Load<sup>4</sup></th> <th>Bits</th> <th>Channels</th></tr>
      *     <tr><td>@ref TF_Unknown</td> <td>D3DFMT_UNKNOWN</td> <td>(unknown)</td> <td>None</td></tr>
      *     <tr><td>@ref TF_RGB5</td> <td>D3DFMT_X1R5G5B5</td> <td>GL_RGB5</td> <td>Y/Y/N</td> <td>15/16</td> <td>RGB</td></tr>
      *     <tr><td>@ref TF_RGB5A1</td> <td>D3DFMT_A1R5G5B5</td> <td>GL_RGB5_A1</td> <td>Y/Y/N</td> <td>16</td> <td>RGBA</td></tr>
      *     <tr><td>@ref TF_RGB8</td> <td>D3DFMT_X8R8G8B8</td> <td>GL_RGB8</td> <td>Yes</td> <td>24</td> <td>RGB</td></tr>
      *     <tr><td>@ref TF_RGBA8</td> <td>D3DFMT_A8R8G8B8</td> <td>GL_RGBA8</td> <td>Yes</td> <td>32</td> <td>RGBA</td></tr>
      *     <tr><td>@ref TF_RGB10A2</td> <td>D3DFMT_A2R10G10B10</td> <td>GL_RGB10_A2</td> <td>Y/Y/N</td> <td>32</td> <td>RGBA</td></tr>
      *     <tr><td>@ref TF_RGBA16F<sup>1</sup></td><td>D3DFMT_A16B16G16R16F</td> <td>GL_RGBA16F</td> <td>N/Y/Y</td> <td>64</td> <td>RGBA</td></tr>
      *     <tr><td>@ref TF_RGBA32F<sup>1</sup></td><td>D3DFMT_A32B32G32R32F</td> <td>GL_RGBA32F</td> <td>N/Y/Y</td> <td>128</td> <td>RGBA</td></tr>
      *     <tr><td>@ref TF_D16</td> <td>D3DFMT_D16F</td> <td>GL_DEPTH_COMPONENT16</td> <td>N/Y-ish<sup>2</sup>/N</td> <td>16</td> <td>R<sup>3</sup></td></tr>
      *     <tr><td>@ref TF_D32</td> <td>D3DFMT_D32F</td> <td>GL_DEPTH_COMPONENT32</td> <td>N/Y-ish<sup>2</sup>/N</td> <td>32</td> <td>R<sup>3</sup></td></tr>
      * </table>
      * <br/>
      * <em><sup>1</sup>:</em> floating-point texture formats (@ref TF_RGBA16F and @ref TF_RGBA32F) may not be supported on
      *     all hardware. Any features requiring these formats should provide an alternate render path for old hardware
      *     (quality may suffer).
      * <br/>
      ...
      * <br/>
      * <em><sup>4</sup>:</em> Texture loading is restricted by the file system library. The values given apply only to
      *     Voodoo/Filesystem, which uses DevIL. Only 8, 16 and 32 bpc formats can be copied into by
      *     IImage::CopyImageData(); DevIL lacks the values to describe packed formats and depth formats are not yet fully
      *     supported in the file system module. The supported formats are more widely supported in other areas of Voodoo,
      *     so it is highly recommended you favor these whenever possible. It @e is, however, possible to load images that
      *     are in other formats and convert them.
      *
      * @section voodoo_textures_depth Depth Buffers
      *
      * For an adapter to be considered fully Voodoo compatible, it must supply realtime depth data to a shader-readable
      * texture. How this is performed is left to the discretion of the adapter, as it varies greatly between old D3D
      * versions and OpenGL or newer D3D. This data should be stored such that greater distances are represented by higher
      * values and the distance should be (if possible) not normalized (not bounded by @p (0-1)). When possible, it is
      ...
      * memory, or other data. They should not be used. As DirectX 9 does not usually allow @ref TF_D32 textures to be read,
      * this may require rendering to a second target and using a @ref TF_RGBA32F texture. Two-pass or two-target rendering
      * can both meet this requirement neatly. Using two passes is slower but more compatible, so this should be made an
      * option if possible.
      *
      * @section voodoo_textures_loading Texture Loading
      *
      * Texture loading is implemented through an IAdapter callback, so Voodoo textures can be created with a variety of
      * options or using any number of functions. To simplify getting texture data into memory in the first place, the
      * Voodoo filesystem library includes specialized IImage classes. These use DevIL as the loading library, so support a
      * ridiculous number of formats. Among the most useful formats are DDS, PNG and TGA, all supporting high resolutions
      * and transparency. Other formats may not be as suitable; JPEG and BMP for example are very lossy and large,
      * respectively.
      *
      * @section voodoo_textures_fileformats Supported File Formats
      *
      * Below is provided a full list of the file formats Voodoo/Filesystem is capable of loading and saving (via DevIL).
      * This list is taken from the DevIL documentation.
      *
      * <table>
      ...
      * </table>
      * <br/>
      * <em><sup>1</sup>:</em> These formats support animated textures, but animations are not supported within Voodoo. Only
      *     the first frame will be used.
      * <br />
      * <em><sup>2</sup>:</em> These formats are not recommended for use, due to poor quality or capabilities.
      * <br />
      ...
      * formats. To use images in these formats, you should load the image, then call IImage::CopyImageData() and provide
      * the desired format. DevIL will handle conversion internally. This may be slow and so should not be used often, but
      * provides a way to load almost any format into Voodoo. The above list of formats is quite large, and some formats
      * are obscure or not well suited for use with Voodoo. However, DevIL supports them and there is no reason to add
      * artificial limitations to Voodoo when it takes less work to provide a more flexible structure. Thus, these formats
      * are all supported and may be used as desired. Some of the more unusual formats may have some innate restrictions or
      * lose data during conversion to Voodoo-supported formats, so take care to properly test any image resources you use.
      */
}
