...
HRESULT CXmlLogger::Log(DWORD Level, BSTR pModule, BSTR pMsg, SAFEARRAY * pArgs)
{
    if ( m_LogFile.m_hFile == CFile::hFileNull ) return VSFERR_NO_FILE;
...
HRESULT CXmlLogger::LogList(DWORD Level, BSTR pModule, BSTR pFormat, VARIANT pList)
{
    if ( m_LogFile.m_hFile == CFile::hFileNull ) return VSFERR_NO_FILE;
