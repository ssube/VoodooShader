...
public:
	...
	{
		VoodooCore->GetLog()->Format("Voodoo GEM: Creating FakeDevice from %d and %d.").With(object).With(device).Done();
	...
	HRESULT _stdcall SetVertexShader(DWORD shaderHandle)
	{		
		VoodooCore->GetLog()->
	...
	HRESULT _stdcall GetVertexShader(DWORD * shaderHandle) 
	{
		VoodooCore->GetLog()->
...
public:
		...
		DWORD flags, D3DPRESENT_PARAMETERS8 * presentparams, IDirect3DDevice8 ** device) 
	{
...
#ifdef _DEBUG
		VoodooCore->GetLog()->Format("Voodoo GEM: Backbuffer parameters: %d by %d (%d buffers), format %d.\n")
			.With(pp.BackBufferWidth).With(pp.BackBufferHeight).With(pp.BackBufferCount).With(pp.BackBufferFormat).Done();
#endif
		...
		*device = fdevice;
		...
		//VoodooCore->SetAdapter((VoodooShader::Adapter*)VoodooDX9);
