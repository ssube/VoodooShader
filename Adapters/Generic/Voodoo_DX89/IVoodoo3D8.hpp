...
public:
  ...
  )
 {
  ...
  VoodooLogger->Log
   (
    "Voodoo DX8.9: IVoodoo3D8::CheckDepthStencilMatch(%d, %d, %d, %d, %d) == %d\n", Adapter, DeviceType,
  ...
  )
 {
  ...
  VoodooLogger->Log
   (
    "Voodoo DX8.9: IVoodoo3D8::CheckDeviceFormat(%d, %d, %d, %d, %d, %d) == %d\n", Adapter, DeviceType,
  ...
  )
 {
  ...
  VoodooLogger->Log
   (
    "Voodoo DX8.9: IVoodoo3D8::CheckDeviceMultiSampleType(%d, %d, %d, %d, %d) == %d\n", Adapter, DeviceType,
  ...
  )
 {
  ...
  VoodooLogger->Log
   (
    "Voodoo DX8.9: IVoodoo3D8::CheckDeviceType(%d, %d, %d, %d, %d) == %d\n", Adapter, CheckType, DisplayFormat,
  ...
  )
 {
  ...
  VoodooLogger->Log
   (
    "Voodoo DX8.9: Backbuffer parameters for new device: %d by %d (%d buffers), %s.\n",
  ...
  VoodooLogger->Log
   (
    "Voodoo DX8.9: IVoodoo3D8::CreateDevice(%d, %d, %d, %d, %d, %d) == %d\n", Adapter, DeviceType,
  ...
  else
  {
   ...
   texture_ThisFrame = VoodooDX89->CreateTexture
    (
 ...
 /**
  * @note Test function: MGE uses a caching of actual D3D8 caps, let's see if we
  * can just grab the appropriate entries from a D3D9 caps struct. They share an
  * identical format until the end of the D3D8 caps section, so...
  * STDMETHOD(GetDeviceCaps) ( UINT Adapter, D3DDEVTYPE DeviceType, D3DCAPS8 *
  * pCaps ) { D3DCAPS9 realCaps;
  * HRESULT hr = mRealObject->GetDeviceCaps(Adapter, DeviceType, &realCaps);
  * #ifdef _DEBUG VoodooLogger->Log("Voodoo DX8.9: IVoodoo3D8::GetDeviceCaps(%d,
  * %d, %d) == %d\n", Adapter, DeviceType, pCaps, hr);
  * #endif if ( SUCCEEDED(hr) ) { ::CopyMemory(pCaps, &realCaps, sizeof(D3DCAPS8));
  * } return hr;
  * }
  */
