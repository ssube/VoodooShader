 ...
 public IDirect3D8
{
 ...
 /**
  * A pointer to the true underlying IDirect3D9 object wrapped by this IVoodoo3D8
  * interface.
  */
...
public:
 ...
 // IDirect3D8 methods ;
 // Determines whether or not a depth-stencil format is compatible with a render
 // target format in a particular display mode. @param Adapter Ordinal number
 // denoting the display adapter to query. D3DADAPTER_DEFAULT is always the primary
 // display adapter. @param DeviceType Member of the D3DDEVTYPE enumerated type,
 // identifying the device type. @param AdapterFormat Member of the D3DFORMAT
 // enumerated type, identifying the format of the display mode into which the
 // adapter will be placed. @param RenderTargetFormat Member of the D3DFORMAT
 // enumerated type, identifying the format of the render target surface to be
 // tested. @param DepthStencilFormat Member of the D3DFORMAT enumerated type,
 // identifying the format of the depth-stencil surface to be tested. @return
 // D3D_OK if the format combination is valid, other values if invalid. @note Will
 // log in debug builds.
 ...
 {
  HRESULT hr = mRealObject->CheckDepthStencilMatch
   (Adapter, DeviceType, AdapterFormat, RenderTargetFormat, DepthStencilFormat);

  VoodooLogger->Log
   (
    LL_Debug, VOODOO_GEM_NAME, "IVoodoo3D8::CheckDepthStencilMatch(%d, %d, %d, %d, %d) == %d", Adapter,
     DeviceType, AdapterFormat, RenderTargetFormat, DepthStencilFormat, hr
   );
  ...
  )
 {
  ...
  VoodooLogger->Log
   (
    LL_Debug, VOODOO_GEM_NAME, "IVoodoo3D8::CheckDeviceFormat(%d, %d, %d, %d, %d, %d) == %d", Adapter,
     DeviceType, AdapterFormat, Usage, RType, CheckFormat, hr
   );
 ...
 /**
  * @note Direct3D8 doesn't seem to support the concept of multisampling quality
  * levels, or won't recognize them in this function. They are not passed back
  * because of this.
  */
  ...
  )
 {
  ...
  VoodooLogger->Log
   (
    LL_Debug, VOODOO_GEM_NAME, "IVoodoo3D8::CheckDeviceMultiSampleType(%d, %d, %d, %d, %d) == %d", Adapter,
     DeviceType, SurfaceFormat, Windowed, MultiSampleType, hr
   );
  ...
  )
 {
  ...
  VoodooLogger->Log
   (
    LL_Debug, VOODOO_GEM_NAME, "IVoodoo3D8::CheckDeviceType(%u, %u, %u, %u, %u) == %d", Adapter, CheckType,
     DisplayFormat, BackBufferFormat, Windowed, hr
   );
  ...
  )
 {
  ...
  VoodooLogger->Log
   (
    LL_Debug, VOODOO_GEM_NAME, "Backbuffer parameters for new device: %u by %u (%u buffers), %s.",
  ...
  HRESULT hr = mRealObject->CreateDevice
   (Adapter, DeviceType, hFocusWindow, BehaviorFlags, &mpPresentationParameters, &mRealDevice);

  VoodooLogger->Log
   (
    LL_Debug, VOODOO_GEM_NAME, "IVoodoo3D8::CreateDevice(%d, %d, %p, %d, %p, %p) == %d", Adapter, DeviceType,
     hFocusWindow, BehaviorFlags, &mpPresentationParameters, mRealDevice, hr
   );

  if (!SUCCEEDED(hr))
  {
   VoodooLogger->Log(LL_Debug, VOODOO_GEM_NAME, "IVoodoo3D8::CreateDevice failed with error %d on adapter %d.", hr, Adapter);
   return hr;
  }
  else
  {
 ...
 STDMETHOD (EnumAdapterModes) (UINT Adapter, UINT Mode, D3DDISPLAYMODE * pMode)
 {
  ...
  VoodooLogger->Log(LL_Debug, VOODOO_GEM_NAME, "IVoodoo3D8::EnumAdapterModes(%d, %d, %p) == %d", Adapter, Mode, pMode, hr);
 ...
 STDMETHOD_ (UINT, GetAdapterCount) ()
 {
  ...
  VoodooLogger->Log(LL_Debug, VOODOO_GEM_NAME, "IVoodoo3D8::GetAdapterCount() == %d", r);
 ...
 STDMETHOD (GetAdapterDisplayMode) (UINT Adapter, D3DDISPLAYMODE * pMode)
 {
  ...
  VoodooLogger->Log(LL_Debug, VOODOO_GEM_NAME, "IVoodoo3D8::GetAdapterDisplayMode(%d, %p) == %d", Adapter, pMode, hr);
 ...
 STDMETHOD (GetAdapterIdentifier) (UINT Adapter, DWORD Flags, D3DADAPTER_IDENTIFIER8 * pIdentifier)
 {
  ...
  VoodooLogger->Log
   (
    LL_Debug, VOODOO_GEM_NAME, "IVoodoo3D8::GetAdapterIdentifier(%u, %u, %p) == %d", Adapter, Flags,
     pIdentifier, hr
   );
 ...
 STDMETHOD_ (UINT, GetAdapterModeCount) (UINT Adapter)
 {
  ...
  VoodooLogger->Log(LL_Debug, VOODOO_GEM_NAME, "IVoodoo3D8::GetAdapterModeCount(%d) == %d", Adapter, r);
 ...
 STDMETHOD_ (HMONITOR, GetAdapterMonitor) (UINT Adapter)
 {
  ...
  VoodooLogger->Log(LL_Debug, VOODOO_GEM_NAME, "IVoodoo3D8::GetAdapterMonitor(%u) == %p", Adapter, hm);

  return hm;
 }

 /**
  * @todo DX9 caps and DX8 caps have identical structure for sizeof(D3D8CAPS), but
  * ::CopyMemory provides incorrect caps that usually result in a crash. Diagnose
  * and correct.
  */
  ...
  *pCaps = d3d8Caps;
  ...
  VoodooLogger->Log(LL_Debug, VOODOO_GEM_NAME, "IVoodoo3D8::GetDeviceCaps(%u, %u, %p) == %d", Adapter, DeviceType, pCaps, hr);

  return D3D_OK;
 }

 /**
  * This is a legacy function to register a software renderer into the DX8 system;
  * however, the DX8 docs state that it is unsupported. The DX9 docs revise that,
  * reintroducing support for it. Since the Voodoo IVoodoo3D8 layer targets DX9,
  * the function is callable and will target the appropriate DX9 function. It will
  * log a warning message, due to technically being illegal.
  */
 STDMETHOD (RegisterSoftwareDevice) (void *pInitializeFunction)
 {
  ...
  VoodooLogger->Log(LL_Debug, VOODOO_GEM_NAME, "IVoodoo3D8::RegisterSoftwareDevice(%p) == %d", pInitializeFunction, hr);

  VoodooLogger->Log
   (
    LL_Warning, VOODOO_GEM_NAME,
